...
#### Scenario: Unknown shop returns 404

- GIVEN a shop domain that does not exist in `fluxbot_dev.shops` and cannot be auto-upserted (e.g. disabled shop lookup in this scenario)
- WHEN a POST is sent with `shop=unknown-store.myshopify.com` and a valid `message`
- THEN the response status is `404`
- AND no row is inserted in `fluxbot_dev.conversations`
