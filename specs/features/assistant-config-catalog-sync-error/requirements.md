...
- Related: `REQ-IA-AI-001`, `REQ-IA-SHOPIFY-001`, `REQ-IA-SHOPIFY-010`
...
- **FR-ACCS-FE-008:** When the App Proxy fallback catalog (`ProductProjection`) returns products, those products MUST be attached as contextual hints to the IA request, not used to replace the shopper's original question or force a catalog-list answer.
- **FR-ACCS-FE-009:** The persisted assistant message and API response `message` MUST remain the same resolved IA answer after contextual augmentation, unless the shopper explicitly asked for product recommendations.
...
6. Given backend IA returns “no tengo el catálogo” and proxy fallback finds products, when `/apps/fluxbot/chat` responds, then the message remains the AI's answer to the shopper's question and `metadata.catalogSource` is `shopify_proxy_catalog_fallback`.
7. Given proxy fallback products were used, when assistant message is persisted, then DB `conversationMessage.content` matches the AI response and the fallback products are retained only as context metadata.
